...
{
    public class LMLog : IDisposable
    {
        public static String s_LogPath = null;
        public static bool s_Close = false;
        public static LMLog s_log = null;

        private static Dictionary<String, LMLog> s_dicLog = new Dictionary<string, LMLog>();


        private String m_strLogKey = "";
        ...
        public static void SetLogKey(String key)
        {
            ...
            if (s_log != null)
            {
                s_log.m_strLogKey = key;
            }
        }
        /// <summary>
        /// 系统日志
        /// </summary>
        public static void Writelog(string logStr, MsgType type = MsgType.Information, String logKey = "")
        {
            if (s_Close)
            {
                return;
            ...
            {
                s_log.Write(new Msg(DateTime.Now, logStr, type));

            }
            else
            {
                s_log.Write(new Msg(DateTime.Now, logKey + "-" + logStr, type));
            }

            if (type != MsgType.Error)
            {
                Console.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " " + logStr);
            }
            else
            {
                Console.Error.WriteLine(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " " + logStr);
            }
        }
        ...
        public static void InitLog()
        {
            ...
            {
                path = AppDomain.CurrentDomain.BaseDirectory.ToString() + "/log/";
            ...
            if (s_log == null)
            {
                if (!String.IsNullOrEmpty(s_LogPath))
                {
                    s_log = new LMLog(path, LogType.None, "");
                }
                else
                {
                    s_log = new LMLog(path, LogType.Daily, "");
                }
            }
        }
        ...
        public static void SaveLog()
        {
            ...
            foreach (KeyValuePair<String, LMLog> log in s_dicLog)
            {
                log.Value.Dispose();
                s_dicLog.Remove(log.Key);
            }
        }
        ...
        /// <param name="t">日志文件创建方式的枚举</param>
        public LMLog(string p, LogType t, String logKey = "")
        {
            m_strLogKey = logKey;
            if (msgs == null)
            {
        ...
        private string GetFilename()
        {
            ...
            if (m_strLogKey.Length > 0)
                return m_strLogKey + "-" + now.ToString(format);
            else
        ...
        private void FileWrite(Msg msg)
        {
            try
            {
                ...
                //
                {
                    ...
                    writer.Write(msg.Datetime.ToString("yyyy-MM-dd HH:mm:ss"));
        ...
        private void FileOpen()
        {
            ...
            if (type == LogType.None && !String.IsNullOrEmpty(s_LogPath))
            {
                UTF8Encoding utf8 = new UTF8Encoding(false);
                writer = new StreamWriter(path, true, utf8);
            }
            else
            {
                UTF8Encoding utf8 = new UTF8Encoding(false);
                writer = new StreamWriter(path + GetFilename(), true, utf8);
            }
        }
